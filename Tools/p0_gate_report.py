...
def _render_order_audit(result: order_audit.OrderAuditResult | None) -> str:
    ...
    if result is None:
        lines.extend([
            "- Order plan was not supplied; crossover assignment order was not independently verified.",
            "- Supply `--order-plan` with the CSV generated by `p0_order_plan.py` for crossover batches.",
        ])
        return "\n".join(lines)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--order-plan",
        type=Path,
        help="Counterbalanced CSV generated by p0_order_plan.py; recommended for crossover batches",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (GateReportError, batch.BatchError, order_audit.OrderAuditError) as exc:
